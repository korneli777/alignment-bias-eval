...
For each model in the probing subset:
  ...
  5. For each (benchmark, prompt_mode, method): re-run the benchmark with
     ProjectionHook attached at the requested layer depths.

Resume-safe per (model, attribute, benchmark, prompt_mode, method, layer).

Usage:
    python scripts/run_intervention.py
    python scripts/run_intervention.py --family llama
    python scripts/run_intervention.py --models meta-llama/Llama-3.1-8B \\
...
# Heavy imports (numpy / torch / transformers / biaseval) are deferred to
# main() so `--help` works without a GPU install.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--attributes",   nargs="+", default=["gender"],
                   choices=["gender", "race"])
    p.add_argument("--benchmarks",   nargs="+", default=["crows_pairs"],
                   choices=["crows_pairs", "stereoset"])
...
def _depths_to_layer_indices(depths: list[float], num_layers: int) -> list[int]:
    ...
    return sorted({max(0, min(num_layers - 1, round(d * (num_layers - 1))))
                   for d in depths})
...
def main() -> int:
    ...
    def load_or_fit_projection(
        spec, results_root, attribute, method, layer_idx, activations, labels,
        *, max_iter, chance_threshold,
    ):
        """Return (projection, bias, metadata). Cached per (model, attr, method, layer)."""
        proj_path = projection_path(
            results_root, spec, attribute, method, layer_idx=layer_idx,
        )
        if proj_path.exists():
            with np.load(proj_path, allow_pickle=True) as npz:
                P = npz["projection"].astype(np.float32)
                bias = (npz["bias"].astype(np.float32)
                        if "bias" in npz.files and npz["bias"].size else None)
                meta = (json.loads(str(npz["metadata"]))
                        if "metadata" in npz.files else {})
            return P, bias, meta
        ...
        if method == "inlp":
            ...
            meta = {
                "method": "inlp", "layer_idx": layer_idx,
    ...
    logger.info(
        "Will intervene on %d models × %d attributes × %d benchmarks × "
    ...
    for spec in specs:
        ...
        for attr in args.attributes:
            ...
            for L in layer_idxs:
                ...
                if X.shape[0] != labels.shape[0]:
                    logger.error(
                        "[error] activation/label size mismatch %s vs %s for %s/%s/L%d",
                        X.shape, labels.shape, spec.model_id, attr, L,
                    )
                    n_errors += 1
                    continue
                for method in args.methods:
                    P, bias, meta = load_or_fit_projection(
        ...
        if args.validate_only:
            for (attr, L, method), info in sanity_cpu.items():
                report_path = (
                    results_root / "intervention_projections" / spec.short_name
    ...
    logger.info("Done. completed=%d skipped=%d errors=%d",
                n_done, n_skipped, n_errors)

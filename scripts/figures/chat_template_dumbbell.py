...
def _layout(df: pd.DataFrame, *, gap: float = 0.9, mean_row_gap: float = 1.8):
    ...
    for fam in FAMILY_ORDER:
        ...
        y += gap
    return pos, -mean_row_gap, group_top, group_bottom


def _draw_panel(ax, sub: pd.DataFrame, bench: str, pos: dict, mean_y: float,
                group_top: dict, group_bottom: dict, *,
                show_yticks: bool,
                xlim: tuple[float, float],
                xticks: list[float]):
    raw_mean    = sub["raw_delta"].mean()
    native_mean = sub["native_delta"].mean()
    ...
    for f1, f2 in zip(fam_keys[:-1], fam_keys[1:], strict=False):
    ...
    ax.plot([raw_mean, native_mean], [mean_y, mean_y],
            color="#c9c9c9", linewidth=2.0, solid_capstyle="butt", zorder=2)
    ax.scatter(raw_mean, mean_y, s=80, marker="o",
               facecolors="white", edgecolors=MEAN_GRAY, linewidth=2.0, zorder=7)
    ax.scatter(native_mean, mean_y, s=120, marker="o",
               facecolors=MEAN_GRAY, edgecolors="white", linewidth=0.9, zorder=8)
    ...
    if show_yticks:
        ax.set_yticks([mean_y] + [yv for _, yv in used_pairs])
        labels = ["Mean"] + [f"{gen} {size}" for (_, gen, size), _ in used_pairs]
        ax.set_yticklabels(labels, fontsize=13)
        colours = ["#777777"] + [FAMILY_COLOURS.get(fam, "black")
                                  for (fam, _, _), _ in used_pairs]
        weights = ["bold"] + ["normal"] * len(used_pairs)
...
def make_figure(df: pd.DataFrame) -> Path:
    ...
    pos, mean_y, g_top, g_bot = _layout(df)
    ...
    for i, (ax, bench) in enumerate(zip(axes, ["stereoset", "crows_pairs"], strict=True)):
        ...
        if sub.empty:
            ...
            continue
        _draw_panel(ax, sub, bench, pos=pos, mean_y=mean_y,
                    group_top=g_top, group_bottom=g_bot,

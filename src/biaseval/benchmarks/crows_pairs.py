...
def run(
    ...
    *,
    ...
    for i, row in enumerate(tqdm(rows, desc=f"CrowS-Pairs [{spec.short_name}/{prompt_mode}]")):
        ...
        # `stereo_won` is length-normalised per-token (default). `stereo_won_raw`
        # is the raw log-prob sum used by lm-evaluation-harness, kept so numbers
        # match published Llama / Mistral CrowS-Pairs references.

"""LLM Implicit Association Test (Bai et al. 2025).

Measures association strength between target concepts (e.g. Black/White) and
attribute words (e.g. pleasant/unpleasant). Reports two effect sizes per test:

    {key}_d            log-prob WEAT d on per-token associations
                       s(t, a) = mean log P(a | "Word that comes to mind for t:").
    {key}_d_weat_embed classical Caliskan WEAT d on embedding-cosine
                       associations; comparable to the static-embedding
                       literature.

Both use d = (mean a - mean b) / std(concat(a, b)), differing only in how the
per-stimulus association is computed.

By default loads the full Bai et al. 2025 stimulus release (21 tests, 5
categories) from https://github.com/baixuechunzi/llm-implicit-bias. Pass
`tests=DEFAULT_IAT_TESTS` to use the small built-in placeholder set instead.
"""
...
# Offline placeholder used by smoke tests; real runs fetch the full Bai
# et al. release via `data.load_iat_stimuli()`.

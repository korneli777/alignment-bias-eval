"""Sanity checks for an intervention before trusting its downstream bias score.

Two automated checks here:
  1. Probe nullification: a fresh probe on post-projection activations should
     no longer recover the attribute (cv accuracy <= chance_threshold).
  2. LM ability: token-level perplexity on neutral text should not blow up
     (post/pre ratio <= blowup_factor). Aggressive projections that wreck
     language modelling produce uninterpretable bias scores.

A third mechanism-specificity check (ablating attribute A should hurt A's
benchmark more than B's) lives in the analysis layer, not here.
"""
...
# Short neutral sentences for the perplexity check. Fixed set so the pre/post
# comparison is paired on identical text.
...
def lm_perplexity(
    ...
    *,
    ...
    """Token-weighted perplexity over a list of short sentences."""

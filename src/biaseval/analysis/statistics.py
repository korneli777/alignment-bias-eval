"""Bootstrap CIs, paired-permutation tests, Cohen's d, and Holm-Bonferroni.

Per-pair tooling produces one row per (base, instruct):
    base | instruct | delta | 95% CI(delta) | Cohen's d (paired) | p (perm)
with Holm correction applied across pairs. The paired-permutation test uses
per-item outcomes (e.g. each CrowS-Pairs item) rather than per-pair means.

Conventions:
    `cohens_d_label`  Cohen's bands: <0.2 negligible, 0.2-0.5 small,
                      0.5-0.8 medium, >=0.8 large.
    `reject_holm`     alpha = 0.05.
    `reject_strict`   alpha = 0.0025 (the stricter NLP-rigor recommendation).
"""
...
def pair_significance_table(
    ...
    *, prompt_mode: str = "raw", scoring: str = "norm",
    ...
    #   reject_strict — α = 0.0025 (NLP statistical-rigor work argues this
    #                   is the threshold needed to keep false-positive rate
    #                   acceptable in NLP comparisons).
    ...
    # Effect-size first: annotate, reorder, and sort by |d|.
...
def cross_benchmark_consistency(
    ...
    *,
    ...
    """Cross-benchmark agreement on the alignment effect (multi-benchmark robustness check).
    ...
    (consistency_df, corr_df)
        ...
        corr_df: Spearman correlation matrix of the per-pair Δ across
            ...
            (cf. recent work on benchmark independence in bias evaluation).
    """
    rows: list[dict[str, Any]] = []
    for base_id, instruct_id, family, generation, size in registry_pairs:
        row: dict[str, Any] = {
    ...
    *, prompt_mode: str = "raw",
    ...
    (consistency_df, corr_df)
        ...
        corr_df: Spearman correlation matrix of the per-pair Δ across
            ...
            near 0 → effect is language-specific.
    """
    rows: list[dict[str, Any]] = []
    for base_id, instruct_id, family, generation, size in registry_pairs:
        row: dict[str, Any] = {
